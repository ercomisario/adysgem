...
class Reports extends CI_Controller
{
    ...
    public function getCustomerReport()
    {
        ...
        $totals = 0;
        $sales = Sale::find_by_sql("SELECT * FROM `sales` WHERE in_activo=1 AND client_id = '$client_id' AND created_at between '$start' AND '$end' ORDER BY created_at");
    ...
    public function getProductReport()
    {
        ...
        $result = '<table id="Table" class="table table-striped table-bordered" cellspacing="0" width="100%"><thead><tr><th>' . label("invoice") . '</th><th>' . label("ProductName") . '</th><th>' . label("Cost") . '</th><th>' . label("Price") . '</th><th>' . label("Quantity") . '</th><th>' . label("tax") . '</th><th>' . label("Total") . '</th><th>' . label("Profit") . '</th></tr></thead><tbody>';
        ...
        foreach ($prducts as $prd) {
            ...
            <td>' . number_format((float)$total, $this->setting->decimals, ',', '.') . '</td></tr>';
            $totalprofit += $total;
        }

        $result .= '</tbody></table><h1>'.label("TotalProfit").' : <span class="ReportTotal" id="totalProduct">'.number_format((float)$totalprofit, $this->setting->decimals, ',', '.').' </span></h1>';
    ...
    public function getRegisterReport()
    {
        ...
        $result .= '</tbody></table><h1>' . label("TotalRevenue") . ' : <span class="ReportTotal">' . number_format((float)$TotalRevenue, $this->setting->decimals, ',', '.'). '</span></h1>';
